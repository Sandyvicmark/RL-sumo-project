...
class CustomEnv(gym.Env):
    def __init__(self, _):
        ...
        self.action_space = gym.spaces.Box(low=-3 * np.ones((N_RL_CAR,)),
                                           high=2 * np.ones((N_RL_CAR,)),
                                           shape=(N_RL_CAR,),
                                           dtype=np.float)
    ...
    def step(self, action):
        ...
        if '0' in traci.simulation.getCollidingVehiclesIDList():
            _ = self.reset()
            self.done = True
        ...
        if not self.is_test and self.n_step % 300000 == 0:
            ...
            df.to_csv('/home/sandymark/StableBaselines_TrainResult/' + time_str + '.csv', index=0)

        if self.is_test and self.n_step % 10000 == 0:
            time_str = time.strftime('%Y%m%d%H-%M-%S', time.localtime(time.time()))
            df_test = pd.DataFrame(np.array(self.record_vmean))
            df_test.to_csv('/home/sandymark/StableBaselines_TestResult/vmean_' + time_str + '.csv', index=0)
            df_test = pd.DataFrame(np.array(self.record_vstd))
            df_test.to_csv('/home/sandymark/StableBaselines_TestResult/vstd_' + time_str + '.csv', index=0)
        return s_, r, self.done, {}

    def reset(self, is_test=False, return_state=True):
        s = []
        if is_test:
            self.is_test = True
        self.done = 0
    ...
    def _get_reward(self, state, action):
        ...
        for idx in range(N_RL_CAR):
            ...
            r_ = 1. - 0 * v_mean / 3. \
                 - (3 * abs(state[3] * 2 - 1)
                    + 0.3 * min(1., max(0, 7 - max(0, state[1] * 173)) / 6.))
    ...
    def _get_density(veh_sq):
        ...
        for idx, car_ in zip(range(len(former_car_list)), former_car_list):
            v_h_for += (1 - math.pow(veh_sq[1][idx] / look_range, 1)) \
                       / max(0.1, traci.vehicle.getSpeed(car_))  # Weighted Harmonic Sum
        ...
        for idx, car_ in zip(range(len(later_car_list)), later_car_list):
            v_h_lat += (1 - math.pow((ROUTE_LENGTH - veh_sq[1][idx - n_later_car]) / look_range, 1)) \
                       / max(0.1, traci.vehicle.getSpeed(car_))
