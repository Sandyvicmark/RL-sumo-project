...
class Train:
    def __init__(self):
        ...
        self.csv_dir = '/home/sandymark/RL-sumo/Output data/test/'
    ...
    def _get_density(veh_sq):
        ...
        for idx in range(len(veh_sq[0])):
            if 0 <= veh_sq[1][idx] < 200:
                former_car_list.append(veh_sq[0][idx])
            elif 240 < veh_sq[1][idx] < 440:
        ...
        for idx, car_ in zip(range(len(later_car_list)), later_car_list):
            ...
            v_h_lat += (1 - math.pow((440 - veh_sq[1][idx - n_later_car]) / 400, 1)) / max(0.1, traci.vehicle.getSpeed(car_))
            w_v_l += (1 - math.pow((440 - veh_sq[1][idx - n_later_car]) / 400, 1))
            ...
            if idx == n_later_car - 1:
                sum_d_lat += 1 / (440 - veh_sq[1][-1])
                # w_p_l += 200
                w_p_l += 1
            else:
                sum_d_lat += (1 - math.pow((440 - veh_sq[1][idx - n_later_car + 1]) / 400, 1)) / max(0.1, veh_sq[1][idx - n_later_car + 1] - veh_sq[1][idx - n_later_car])
                # w_p_l += (200 - (629 - veh_sq[1][idx - n_later_car + 1]))
                w_p_l += (1 - math.pow((440 - veh_sq[1][idx - n_later_car + 1]) / 400, 1))
        ...
        if n_later_car:
            ...
            den_later = 300 * sum_d_lat / w_p_l + 15 / v_h_lat + 500 / max(0.1, (440 - veh_sq[1][-1]))
        else:
            den_later = 0

        print('den_former: ', den_former, 'sdf: ', sum_d_for, 'wpf: ', w_p_f, 'vh: ', v_h_for, 'itv: ', veh_sq[1][0])
        print('den_later: ', den_later, 'sdll: ', sum_d_lat, 'wpl: ', w_p_l, 'vh: ', v_h_lat, 'itv: ', 440-veh_sq[1][-1])
    ...
    def reset(self):
        ...
        for idx in range(len(veh_list)):
            ...
            traci.vehicle.setSpeed(veh_list[idx], 2)
        traci.simulationStep()
    ...
    def run(self):
        ...
        while self.step < EPISODE:                                               # Train start:
            for car_ in dqn_car:
                ...
                traci.vehicle.setSpeed(car_, max(0, self.s[car_][0]+0.2*self.act[car_]))     # 2. Perform this action
            ...
            for car_ in dqn_car:
                ...
                """
                if (round(traci.vehicle.getAcceleration(car_), 3) != round(self.act[car_] * 0.1, 3)) \
                        and next_tls and next_tls[0][3] == 'r':
                    print(car_, 'a_target: ', traci.vehicle.getAcceleration(car_))
                    print(car_, 'a_act: ', self.act[car_] * 0.1)
            ...
            for car_ in self.veh_list:
                if self.step < EMISSION_TEST_TERM:
                    cur_fuel_sum += traci.vehicle.getFuelConsumption(car_) * 0.2
...
if __name__ == '__main__':
    ...
    plt.ion()
    dqn_car = ['999']  #  '9', '19', '29',  '998', '997', '996'], '4', '14', '24', '34'
